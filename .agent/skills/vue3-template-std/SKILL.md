...
6. **善用tailwindcss**：优先使用 `tailwindcss` 提供的工具类，不要重复造轮子。
7. **主题色**：在进行页面设计时，优先使用 `src/assets/styles/theme.scss` 中定义的变量，不要直接使用颜色值，或者使用和当前主题色相近的颜色。
8. **图标规范**：优先使用 `lucide-vue-next` 库中的图标，不要重复造轮子。
9. **善用图标**：善于利用图标和背景色的协调搭配设计
